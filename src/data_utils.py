...
def get_datasets() -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Calcular el IQR
    Q1 = np.percentile(app_test['trip_distance'], 0)
    Q3 = np.percentile(app_test['trip_distance'], 95)
    IQR = Q3 - Q1
    ...
    app_train = app_train.dropna(subset=['passenger_count'])
    app_test = app_test.dropna(subset=['passenger_count'])
